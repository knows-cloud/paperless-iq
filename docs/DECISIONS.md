...
**Rule:** `PaperlessNGXClient` deliberately has **no** entity-creation method — `create_entity()` existed as an unused second implementation and was removed. Don't re-add one: a creation path on the analyze-time client is how "created before the user approved" bugs get written. Analyze-time code may only read (`list_entities` / `list_entities_with_map`); the analyzer's mock is built with `spec=PaperlessNGXClient` so a write attempt raises rather than silently passing. Never widen `create_missing` to a bare truthy check — resolve it through `_may_create(create_missing, entity_type)` so an absent key means "don't create". Every new entity type that `_patch_paperless` learns to resolve needs its own key in `creation_policy_map` and its own `*_creation_policy` field on `PaperlessIQConfig`; a type with no policy defaults to False, never True. Any new approve() caller must decide explicitly between the bool (human authority) and the mapping (policy authority) — the default is `False`, which creates nothing.

---

## D-26 · The PR audit gate blocks *new* advisories; a scheduled sweep watches main

**Decision:** `npm audit --audit-level=low` no longer runs as a blocking PR step. The PR gate is `.github/scripts/check-npm-audit-delta.py`, which audits the base lockfile and the head lockfile and fails only on advisories present in head but absent in base. Advisories already affecting `main` are handled by `.github/workflows/audit-sweep.yml`, a daily job that runs the full `npm audit` plus `pip-audit` against `main` and opens (or comments on) a tracking issue. The audit step also moved to **last** in the frontend job, after `tsc`, `eslint` and `check:i18n`.

**Rationale:** A blanket `npm audit` in a PR job asks "does the world currently contain any advisory against our tree?" — a question whose answer changes without anyone touching the repo. When GHSA-2v37-7h3g-55p8 (`nanoid <3.3.17`) was published on 2026-08-08, all twelve open PRs went red simultaneously, including Python-only bumps that touch no JavaScript, and including #170 which carried the fix. Because the audit step ran *second*, before `tsc`/`eslint`/`check:i18n`, and the job is `-e`, those three gates never executed at all: for six days twelve PRs reported a security failure while their type and lint status was simply unknown. A gate that hides other gates is worse than it looks. The old setup was also *too quiet* in the other direction — an advisory published while no PR was open went unnoticed until someone happened to open one, which is exactly how nanoid sat for six days.

This is not a relaxation of the security bar. A PR that introduces a vulnerable dependency is still blocked immediately, at any severity — that path is verified by test. What changed is that pre-existing advisories became their own tracked work item instead of a tax on every unrelated PR, and they are now surfaced within a day rather than whenever a PR happens to be opened.

**Rule:** Do not restore a bare `npm audit` as a blocking PR step, and do not move the audit step earlier in the job — the ordering is load-bearing, not cosmetic. `check-npm-audit-delta.py` **fails closed**: if the base manifest/lockfile cannot be read, or either audit is unparseable, it exits non-zero rather than passing silently; keep that behaviour. Advisory identity is `(package, via.source)` — string entries in `via` are pointers to other packages, not advisories, and are skipped on purpose. If the sweep's tracking issue is closed while advisories remain, the next run opens a fresh one; fix `main` rather than muting the job.

---

## D-27 · Provider settings resolve per role, and an empty role field inherits

**Decision:** The LLM, embedding and reranker roles each carry their own `{provider, base_url, credentials, model}`. `build_providers()` returns a `ProviderRegistry` with `for_llm()` / `for_embed()` / `for_rerank()` accessors instead of a one-entry dict keyed by provider name. **Every per-role field defaults to empty, and empty means "inherit from the LLM section"** — `embed_base_url`, `embed_credentials`, `rerank_base_url` and `rerank_api_key` all follow this rule. A role builds its own provider instance only when it actually overrides something; otherwise it reuses the chat instance. Credentials only ever inherit from a *compatible* chat provider (an OpenAI-style embed role will not inherit Bedrock credentials), and a role that ends up with no usable credentials raises with an actionable message rather than failing at the first request.

**Rationale:** `build_providers` returned `{provider_name: provider}` — a single entry, holding only the chat provider. Every consumer then did `providers.get(config.llm_provider)`, so any other role wanting the same provider *type* was forced to share that one instance, and with it its base URL and its key. That is where `embed_provider='openai' requires llm_provider='openai' as well (credentials are shared)` came from: not a policy, just the registry's shape leaking into a user-facing restriction. It blocked the common self-hosted setup of a local chat model plus a separate GPU box for embeddings, and it left reranking with no endpoint of its own at all (issue #183). The providers themselves were never the problem — `OpenAIProvider.__init__` already accepted `base_url` and `embed_model` per instance.

Inheritance is what makes this safe to ship without a data migration. Settings persist as one JSON blob (`SettingsORM.config_json`), so Pydantic defaults cover existing rows; every install that predates these fields has them all empty and therefore resolves each role exactly where it resolved before.

**Rule:** A new per-role setting defaults to empty and inherits when empty — never copy the LLM value into the role field at save time, or you freeze what should track. Resolve roles through the registry accessors, not `providers.get(<name>)`; the registry still subclasses `dict` for older call sites, but new code must not rely on that. Any new role secret goes through all four `settings_service.py` touchpoints (`CREDENTIAL_FIELDS`, `_BYTES_CREDENTIAL_FIELDS`, `_ENV_MAP`, `*_stored`) and the UI must send `__REDACTED__` when the field is untouched — a blank field means "unchanged", never "erase".

Most important: **the effective embed endpoint is part of the index's identity.** `embed_changed` compares `effective_embed_endpoint()` alongside provider and model, because the same model name served by a different host produces incompatible vectors, and mixing two vector spaces in one collection degrades retrieval with no error and no warning. Credentials are deliberately excluded from that comparison — rotating a key against the same endpoint yields identical vectors and must not prompt a needless re-index. Any future per-role endpoint that feeds embeddings must be added to `effective_embed_endpoint()`.
